...
[Serializable]
public class WardenPlayerActorType : WardenBase
{
    private readonly static string TypeName = "WARDEN Player";
    ...
    {
        return JsonUtility.FromJson<WardenPlayerActorType>(json);
    }
    #endregion
    ...
    public override void Command(string command, ActorData tokenData)
    {
        ...
        else if (command.StartsWith("ModFate"))
        {
            int original = Fate;
            int changeValue = int.Parse(command.Split("|")[1]);
            Fate = Clamped(0, Fate + changeValue, MaxFate);
            int diff = Fate - original;
            if (diff != 0 && tokenData.Placed)
            {
                string plus = diff > 0 ? "+" : "";
                PopoverText.Create(token, $"/{plus}{diff}|_Fate", Color.white);
            }
        }
        ...
        else if (command.StartsWith("UpdateStats"))
        {
            ...
            WardenPlayerActorType lmu = JsonUtility.FromJson<WardenPlayerActorType>(json);
            MaxHP = lmu.MaxHP;
            MaxFate = lmu.MaxFate;
            MaxSP = lmu.MaxSP;
            MaxReaction = lmu.MaxReaction;
            MaxAP = lmu.MaxAP;
            Toughness = lmu.Toughness;
            Combat = lmu.Combat;
            Resolve = lmu.Resolve;
            Special = lmu.Special;
            Perception = lmu.Perception;
            Skill = lmu.Skill;
            Armor = lmu.Armor;
            Speed = lmu.Speed;
    ...
    public override void UpdatePanel(ActorData tokenData, string elementName)
    {
        ...
        panel.Q("Fate").Q<Label>("Pips").text = SymbolString("▰", Fate, MaxFate);
    }
    ...
    public override void InitPanel(ActorData actorData, string elementName, bool selected)
    {
        ...
        VisualElement fate = UI.CreateFromTemplate("UI/TableTop/LancerCoreStat");
        fate.name = "Fate";
        fate.Q<Label>("StatName").text = "FATE POINTS";
        fate.Q<Label>("Pips").style.color = ColorUtility.GetColor("#E4004C");
        fate.Q<Label>("Pips").text = SymbolString("▰", Fate, MaxFate);
        if (data != null)
        {
            fate.Q<Button>("Increment").RegisterCallback<ClickEvent>((evt) => { Player.Self().CmdRequestActorCommand(data.Id, "ModFate|1"); });
            fate.Q<Button>("Decrement").RegisterCallback<ClickEvent>((evt) => { Player.Self().CmdRequestActorCommand(data.Id, "ModFate|-1"); });
        }
        else
        {
            UI.ToggleDisplay(fate.Q<Button>("Increment"), false);
            UI.ToggleDisplay(fate.Q<Button>("Decrement"), false);
        }
        container.Add(fate);
      ...
      */
        ...
        stats.Add($"TOUGHNESS|+{Toughness}");
        stats.Add($"COMBAT|+{Combat}");
        stats.Add($"RESOLVE|+{Resolve}");
        stats.Add($"SPECIAL|+{Special}");
        stats.Add($"PERCEPTION|+{Perception}");
        stats.Add($"SKILL|+{Skill}");
    ...
    private void AlterStatModal()
    {
        ...
        Modal2.AddInlineNumberNudgerField("Toughness", "Toughness", Toughness, 0, 20);
        Modal2.AddInlineNumberNudgerField("Combat", "Combat", Combat, 0, 20);
        Modal2.AddInlineNumberNudgerField("Resolve", "Resolve", Resolve, 0, 20);
        Modal2.AddInlineNumberNudgerField("Special", "Special", Special, 0, 50);
        Modal2.AddInlineNumberNudgerField("Perception", "Perception", Perception, 0, 20);
        Modal2.AddInlineNumberNudgerField("Skill", "Skill", Skill, 0, 20);
        ...
        Modal2.AddFooterConfirm("Save", () =>
        {
            ...
            Toughness = Modal2.GetNumberNudgerFieldValue("Toughness");
            Combat = Modal2.GetNumberNudgerFieldValue("Combat");
            Resolve = Modal2.GetNumberNudgerFieldValue("Resolve");
            Special = Modal2.GetNumberNudgerFieldValue("Special");
            Perception = Modal2.GetNumberNudgerFieldValue("Perception");
            Skill = Modal2.GetNumberNudgerFieldValue("Skill");
