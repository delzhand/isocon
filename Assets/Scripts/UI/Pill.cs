...
public class Pill
{
    ...
    public static VisualElement InitNumber(string name, string text, int number, int max, Color color, bool forToken)
    {
        ...
        if (forToken)
        {
            ...
            {
                Player.Self().CmdRequestActorCommand(Actor.GetSelected().Data.Id, $"IncrementTag|{name}");
            });
            p.Q<Button>("Decrement").RegisterCallback<ClickEvent>((evt) =>
            {
                Player.Self().CmdRequestActorCommand(Actor.GetSelected().Data.Id, $"DecrementTag|{name}");
            });
            p.Q<Button>("Remove").RegisterCallback<ClickEvent>((evt) =>
            {
                Player.Self().CmdRequestActorCommand(Actor.GetSelected().Data.Id, $"RemoveTag|{name}");
            });
        }
        else
        {
